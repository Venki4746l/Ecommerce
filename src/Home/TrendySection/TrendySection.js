...
const TrendySection = (props) => {
  ...
  const { imgSrc, title, price, cross_price } = shopDetails;
    ...
    <div className="border mb-4">
      ...
      <div className="d-flex justify-content-between">
        <div className="bottom-button-container">
          ...
          <a href='/' className="cart-button">{first_button}</a>
        </div>
        <div className="bottom-button-container">
          <svg
            className="icon"
          ...
          <button onClick={()=>dispatch({type:ADD_CART,payload:shopDetails})} className="Addcart-button">{second_button}</button>
        </div>
      </div>
    </div>
  );
